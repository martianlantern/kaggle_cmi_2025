...
# %%
# Advanced feature engineering functions from 5fold notebook
...
def compute_cross_axis_energy(df):
    """Compute energy-based features using FFT"""
    axes = ['x', 'y', 'z']
    features = {}
    for axis in axes:
    ...
    for i, axis1 in enumerate(axes):
        for axis2 in axes[i+1:]:
            features[f'er_c_{axis1}{axis2}'] = np.corrcoef(
                np.abs(fft(df[f'acc_{axis1}'].values)), 
                np.abs(fft(df[f'acc_{axis2}'].values))
            )[0, 1]
    return features

# %%
INPUT_DIR = "../data"
OUTPUT_DIR = "../outputs/feature-v1"
...
class CMIBackbone(nn.Module):
    def __init__(self, imu_dim, thm_dim, tof_dim, **kwargs):
        super().__init__()
        # Calculate split dimensions based on actual data
        imu_acc_dim = 3  # acc_x, acc_y, acc_z or linear_acc versions
        imu_rot_dim = 4  # rot_w, rot_x, rot_y, rot_z
        imu_other_dim = max(1, imu_dim - 7) if imu_dim > 7 else 1  # remaining engineered features
    ...
    def forward(self, imus, thms, tofs):
        # Handle both split format and concatenated format
        if isinstance(imus, list):
            # Split sensor format from CMIFeDataset
            imu_acc = imus[0].permute(0, 2, 1) if len(imus) > 0 else torch.zeros(1, 3, 100).to(self.device)
            imu_rot = imus[1].permute(0, 2, 1) if len(imus) > 1 else torch.zeros(1, 4, 100).to(self.device) 
            imu_other = torch.cat([imu.permute(0, 2, 1) for imu in imus[2:]], dim=1) if len(imus) > 2 else torch.zeros(1, 1, 100).to(self.device)
            ...
            # Process THM sensors
            thm_feats = []
            for i in range(min(5, len(thms))):
                thm_data = thms[i].permute(0, 2, 1)
                if i == 0:
                    thm_feat = self.thm_branch1(thm_data)
                elif i == 1:
                    thm_feat = self.thm_branch2(thm_data)
                elif i == 2:
                    thm_feat = self.thm_branch3(thm_data)
                elif i == 3:
                    thm_feat = self.thm_branch4(thm_data)
                else:
                    thm_feat = self.thm_branch5(thm_data)
                thm_feats.append(thm_feat)
            
            # Process TOF sensors  
            tof_feats = []
            for i in range(min(5, len(tofs))):
                tof_data = tofs[i].permute(0, 2, 1)
                if i == 0:
                    tof_feat = self.tof_branch1(tof_data)
                elif i == 1:
                    tof_feat = self.tof_branch2(tof_data)
                elif i == 2:
                    tof_feat = self.tof_branch3(tof_data)
                elif i == 3:
                    tof_feat = self.tof_branch4(tof_data)
                else:
                    tof_feat = self.tof_branch5(tof_data)
                tof_feats.append(tof_feat)
                
        else:
            # Concatenated format (backward compatibility)
            x = imus  # imus is actually the full tensor
            B, C, L = x.shape
            
            # Split input based on sensor type (simplified approach)
            imu_channels = 7  # Base IMU channels
            ...
            # Split IMU into acc, rot, other
            imu_acc = imu_data[:, :3, :]  # First 3 channels
            imu_rot = imu_data[:, 3:7, :]  # Next 4 channels  
            remaining_imu = x[:, 7:imu_channels, :] if C > 7 else torch.zeros(B, 1, L, device=x.device)
            # If we have remaining IMU channels, use them; otherwise create dummy
            imu_other = remaining_imu if remaining_imu.size(1) > 0 else torch.zeros(B, 1, L, device=x.device)
            # Update the branch to handle the actual dimension
            actual_other_dim = imu_other.size(1)
            ...
            # Process each THM/TOF sensor separately
            thm_feats, tof_feats = [], []
        ...
        thm_feat = self.thm_proj(torch.cat(thm_feats, dim=1)) if thm_feats else torch.zeros(1, 64, 25).to(imu_feat.device)
        tof_feat = self.tof_proj(torch.cat(tof_feats, dim=1)) if tof_feats else torch.zeros(1, 128, 25).to(imu_feat.device)
...
class CMIModel(nn.Module):
    def __init__(self, in_ch: int, n_classes: int, dropout: float = 0.2, **kwargs):
        super().__init__()
        # Calculate dimensions based on actual input channels
        # For now, assume standard split: 17 total (7 IMU + 5 THM + 5 TOF)
        if in_ch <= 17:
            imu_dim = 7  # Base IMU channels 
            thm_dim = 5  # Thermopile channels
            tof_dim = max(1, in_ch - imu_dim - thm_dim)  # Remaining for ToF
        else:
            # Extended feature set - adjust based on typical feature counts
            imu_dim = min(in_ch // 2, 20)  # Cap IMU features
            thm_dim = 5  # Standard thermopile
            tof_dim = max(1, in_ch - imu_dim - thm_dim)
    ...
    def forward(self, x, thm=None, tof=None):  # Handle both formats
        if thm is not None and tof is not None:
            # Split sensor format
            feat = self.backbone(x, thm, tof)
        else:
            # Concatenated format or split format passed as x
            if isinstance(x, list):
                # x is actually [imus, thms, tofs]
                feat = self.backbone(x[0], x[1], x[2])
            else:
                # Backward compatibility - concatenated tensor
